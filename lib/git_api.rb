module Gitlog
  module Base
    ...
    module Config
      ...
      def self.set_git_bin_path=(path)
        @git_bin_path = path
      end

      def self.get_git_bin_path
        @git_bin_path
      end

      def self.set_git_repositories_root=(abs_path)
        @git_repositories_root = abs_path
      end

      def self.get_git_repositories_root
...
Gitlog::Base::Config.set_git_bin_path = '/usr/bin/git'
Gitlog::Base::Config.set_git_repositories_root = "/home/git/repositories"

module Gitlog
  module Base
    module Action
      module Command
        def self.execute(user, repository, options = [])
          @git_bin ||= Gitlog::Base::Config.get_git_bin_path
          @git_repos_root ||= Gitlog::Base::Config.get_git_repositories_root
          git_dir_option = "--git-dir=#{@git_repos_root}/#{user}/#{repository}"

          %x["#{@git_bin} #{git_dir_option} #{options.join(' ')}"]
        end
      end
      ...
      def get_hash_by_path(user, repository, base, path, type='blob')
        Command.execute(user, repository, ['ls-tree', base, '--', path])
...
::T.send :include, Gitlog::Base

class T2 < T
  git_config_with do |c|
    c.git_path = "/usr/bin/git --git-dir=."
  end
#  puts self.methods.sort
end
puts T2.get_hash_by_path('intern', 'git_server', 'master', 'setup.py')
